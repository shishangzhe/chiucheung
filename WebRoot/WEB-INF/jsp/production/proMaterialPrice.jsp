<%@ taglib prefix="p" uri="http://www.chiucheung.cn" %>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<html>
<head>
    <title>物料指导价格管理</title>
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $('#dg').treegrid({
            ...
            url: '${pageContext.request.contextPath}/production/materialPrice/findAllWarMaterialList.action',
            idField: 'id',
            ...
            onBeforeExpand: function (row) {
                //动态设置展开查询的url
                var url = '${pageContext.request.contextPath}/production/materialPrice/findAllWarMaterialList.action?materialCodeForExpand=' + row.materialCode + "&level=" + $(this).treegrid('getLevel', row.id);
                $(this).treegrid("options").url = url;
                return true;
            }, onExpand: function (row) {
                //展开后将url设置为原来的url，否则刷新的时候会使用更改后的url刷新
                var url = '${pageContext.request.contextPath}/production/materialPrice/findAllWarMaterialList.action';
                $(this).treegrid("options").url = url;
    ...
    function openUpdatePrice() {
        ...
        if (select == null) {
            ...
            if (!$.isArray(select.children)) {
                ...
                $.ajax({
                    type: 'POST',//post请求
                    url: '${pageContext.request.contextPath}/production/materialPrice/selectPriceByMaterialId.action',//ajax请求的url
                    data: {
    ...
    function saveOrUpdatePrice() {
        ...
        var url = "${pageContext.request.contextPath}/production/materialPrice/";
    ...
    function changStatue() {
        ...
        if (select.length > 0) {
            $.ajax({
                ...
                async: false,//同步请求
                cache: false,//不缓存此页面
                url: '${pageContext.request.contextPath}/production/materialPrice/updateStatue.action?materialIdList=' + materialIdList,//ajax请求的url
...
<div id="menu" class="easyui-menu" style="width: 80px; display: none;">
    <p:isPrivilege pid="ad" mid="ada">
        <div data-options="iconCls:'icon-add'" onclick="openInsertPrice();">添加物料指导价格</div>
    </p:isPrivilege>
    <p:isPrivilege mid="adb" pid="ad">
        <div data-options="iconCls:'icon-edit'" onclick="openUpdatePrice();">修改物料指导价格</div>
    </p:isPrivilege>
    <p:isPrivilege mid="adc" pid="ad">
        <div data-options="iconCls:'icon-man'" onclick="openAuditDialog()">指导价格审核</div>
    </p:isPrivilege>
