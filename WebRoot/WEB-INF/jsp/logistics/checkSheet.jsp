  ...
  window.onload=window.onresize = function(){
	...
	$(function () {
		$('#dg').treegrid({
		    ...
		    url:'${pageContext.request.contextPath}/logistics/checkSheet/findAllCheckSheetList.action',    
		    idField:'id', 
		    ...
		    onBeforeExpand:function(row){
		    	//动态设置展开查询的url 
		    	/* var url = '${pageContext.request.contextPath}/logistics/checkSheet/findAllCheckSheetList.action'; 
                $(this).treegrid("options").url = url;  
                return true;  */
		    	var url = "";
                //动态设置展开查询的url
                if ($(this).treegrid('getLevel',row.id) == 1){
		    		url = '${pageContext.request.contextPath}/logistics/checkSheet/findAllTravelSpendingRecordsByFinCheckSheetId.action'; 
                }else if ($(this).treegrid('getLevel',row.id) == 2){
                	url = '${pageContext.request.contextPath}/logistics/checkSheet/findAllTravelSpendingRecordsSubsidiaryListByLogTravelSpendingRecordsId.action';
                }
                $(this).treegrid("options").url = url;  
                return true;      
  			...
  			$.messager.confirm('确认对话框', '您确定要删除吗？', function(b){//给用户一个删除几条信息的确认提示框
				if (b){//如何用户点击的是确认
					...
					if ($('#dg').treegrid('getLevel',selected.id) == 1){//是父节点
		  			...
		  			$.ajax({
						type:'POST',//post请求
						url: "${pageContext.request.contextPath}/logistics/checkSheet/deleteCheckSheetById.action",//ajax请求的url
	...
	function exportExcelForCheckSheet(){
		...
		if (selected != null){
			...
			window.open("${pageContext.request.contextPath }/logistics/checkSheet/exportExcelForCheckSheetById.action?id="+id);
		}else{
	...
	function exportExcelForLive(){
		...
		if (selected != null){
			...
			window.open("${pageContext.request.contextPath }/logistics/checkSheet/exportExcelForLiveById.action?id="+id);
		}else{
			showMessager("提示","请选择一条记录进行导出");
		}
	}
	...
	function graphicalReportsForFCF(){
		...
		if ($("#selectChartType").combobox("getValue") == "" || $("#selectChartType").combobox("getValue") == null || $("#selectChartType").combobox("getValue") == undefined){
			...
			if (selected != null){
				...
				$.ajax({
					type:'POST',//post请求
					url: "${pageContext.request.contextPath }/logistics/checkSheet/graphicalReportsForFCFById.action",//ajax请求的url
					data: {id:id},//传的参数,序列化表单
					async: false,//同步请求
					cache: false,//不缓存此页面
					success: function(data){//请求成功后的回调函数。data：服务器返回数据。
	...
	function submitToFinance(){
		var selected = $('#dg').treegrid("getSelected");
		if (selected != null){
			$.messager.confirm('确认对话框', '您确定要递交给财务吗？', function(b){//给用户一个删除几条信息的确认提示框
				if (b){//如何用户点击的是确认
					var id = "";
					//判断是否是子节点
					if ($('#dg').treegrid('getLevel',selected.id) == 1){//是父节点
					...
					$.ajax({
						type:'POST',//post请求
						url: "${pageContext.request.contextPath}/logistics/checkSheet/updateCheckSheetByIdForSubmitToFinance.action",//ajax请求的url
						data: {id:id},//传的参数,序列化表单
						async: false,//同步请求
						cache: false,//不缓存此页面
						success: function(data){//请求成功后的回调函数。data：服务器返回数据。
  	...
  	<div id="gridToolBar" style="padding:5px;height:auto;">
		<div style="display:inline-block;">
			...
			<p:isPrivilege pid="hk" mid="hkb">
				...
				<a href="#" class="easyui-linkbutton" iconCls="icon-redo" plain="true" onclick="exportExcelForLive();" style="float: left;">导出住宿表</a><div class="btn-separator"></div>
			</p:isPrivilege>
			<p:isPrivilege pid="hk" mid="hkc">
			...
			<p:isPrivilege pid="hk" mid="hkd">
				<a href="#" class="easyui-linkbutton" iconCls="icon-large-smartart" plain="true" onclick="submitToFinance();" style="float: left;">递交财务</a><div class="btn-separator"></div>
			</p:isPrivilege>
		</div>
		<div>
			<form id="queryForm">
	...
	<div id="menu" class="easyui-menu" style="width: 80px; display: none;">
	    ...
	    <p:isPrivilege pid="hk" mid="hka">
	   	...
	   	<p:isPrivilege pid="hk" mid="hkb">
	   		...
	   		<div data-options="iconCls:'icon-redo'" onclick="exportExcelForLive();">导出住宿表</div>
	   	</p:isPrivilege>
	   	<p:isPrivilege pid="hk" mid="hkc">
	   		<div data-options="iconCls:'icon-large-chart'" onclick="graphicalReportsForFCF();">图形报表</div>
	   	</p:isPrivilege>
	   	<p:isPrivilege pid="hk" mid="hkd">
	   		<div data-options="iconCls:'icon-large-smartart'" onclick="submitToFinance();">递交财务</div>
	   	</p:isPrivilege>
	</div>
