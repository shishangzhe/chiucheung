...
<html>
  ...
  <script type="text/javascript">
  	$(function(){
		...
		$('#cc').combobox({    
		    url:"${pageContext.request.contextPath}/system/dictionarie/findAllKeywordByDistinct.action",//加载数据的url    
		    ...
		    onChange:function (newValue,oldValue){//选择一行时，根据选择的行给下面的表格加载数据
		    	var url = "${pageContext.request.contextPath}/system/dictionarie/findAllDictionarieByKeyword.action";
  				...
  				if ($("#flag").val() == "true"){//该标识为true，允许删除，为false已有编辑或新增的行未保存，删除后重新加载数据，未保存的数据将丢失
		  			$.messager.confirm('确认对话框', '您确定要删除这条数据吗？', function(b){//给用户一个删除几条信息的确认提示框
						if (b){//如何用户点击的是确认
							$.ajax({
								type:'POST',//post请求
								url: "${pageContext.request.contextPath}/system/dictionarie/deleteSysDictionarieById.action?id="+select.id,//ajax请求的url
								async: false,//同步请求
								cache: false,//不缓存此页面
  			...
  			if (flag == false){
				...
				return;//跳出循环
	  			...
	  			if (changes[0].id == undefined){//表示新增，如果是修改，是能获取到id的
	  				$.ajax({
						type:'POST',//post请求
						url: "${pageContext.request.contextPath}/system/dictionarie/saveSysDictionarie.action",//ajax请求的url
						data: changes[0],//传的参数
						async: false,//同步请求
						cache: false,//不缓存此页面
						success: function(data){//请求成功后的回调函数。data：服务器返回数据。
	  				...
	  				$.ajax({
						type:'POST',//post请求
						url: "${pageContext.request.contextPath}/system/dictionarie/updateSysDictionarie.action",//ajax请求的url
						data: changes[0],//传的参数
						async: false,//同步请求
						cache: false,//不缓存此页面
						success: function(data){//请求成功后的回调函数。data：服务器返回数据。
...
<body>   
	...
	<div id="gridToolBar" style="padding:5px;height:auto">
		<div style="display:inline-block;">
			...
			<p:isPrivilege pid="bd" mid="bda,bdb"> 
