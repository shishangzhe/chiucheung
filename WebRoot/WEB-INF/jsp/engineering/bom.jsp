		  ...
		  $('#dg').treegrid({
			...
			$('#dg1').treegrid({
			    ...
			    onContextMenu: function(e, row) { //右键时触发事件
	            	...
	            	$(this).treegrid('getPanel').panel('panel').focus();
	            }
			});
			...
			$("#dg2").datagrid({
			    ...
			    onDblClickRow: function(index, row) { //双击时触发事件
					...
					$($('#dg1').treegrid('getEditor', {id:editRow.id,field:'productionWorkshop'}).target).textbox('setValue',row.productionWorkshop);
					$($('#dg1').treegrid('getEditor', {id:editRow.id,field:'surfaceTreatment'}).target).textbox('setValue',row.color);
					$($('#dg1').treegrid('getEditor', {id:editRow.id,field:'useUnit'}).target).textbox('setValue',row.useUnit);
					$($('#dg1').treegrid('getEditor', {id:editRow.id,field:'warMaterialId'}).target).textbox('setValue',row.id);
	...
	function collapseAll(){
  		...
  		if (selected != null){
  			$('#dg1').treegrid('insert',{
	  			...
	  			data: {
	  				...
	  				'useUnit':selected.useUnit,
	  				'isMaterial':selected.isMaterial,
	  				'warMaterialId':selected.warMaterialId,
  	...
  	function foreachCopyRowChildren(children,parentId){
  		...
  		$('#dg1').treegrid('append',{
  			...
  			data:[{
  				...
  				'useUnit':children.useUnit,
  				'isMaterial':children.isMaterial,
  				'warMaterialId':children.warMaterialId,
  	...
  	function findWarMaterialByMaterialCode(materialCode){
  		if (materialCode != null && materialCode != ''){
	  		$.ajax({
				type:'POST',//post请求
				url: "${pageContext.request.contextPath}/engineering/bom/findWarMaterialByMaterialCode.action",//ajax请求的url
				...
				success: function(data){//请求成功后的回调函数。data：服务器返回数据。
					if(data.success){
						...
						$($('#dg1').treegrid('getEditor', {id:editRow.id,field:'productionWorkshop'}).target).textbox('setValue',data.bomSubsidiary.productionWorkshop);
						$($('#dg1').treegrid('getEditor', {id:editRow.id,field:'surfaceTreatment'}).target).textbox('setValue',data.bomSubsidiary.surfaceTreatment);
						$($('#dg1').treegrid('getEditor', {id:editRow.id,field:'useUnit'}).target).textbox('setValue',data.bomSubsidiary.useUnit);
						$($('#dg1').treegrid('getEditor', {id:editRow.id,field:'warMaterialId'}).target).textbox('setValue',data.bomSubsidiary.warMaterialId);
						...
						$($('#dg1').datagrid('getEditor', {id:editRow.id,field:'productionWorkshop'}).target).textbox('setValue','');
						$($('#dg1').treegrid('getEditor', {id:editRow.id,field:'surfaceTreatment'}).target).textbox('setValue','');
						$($('#dg1').treegrid('getEditor', {id:editRow.id,field:'useUnit'}).target).textbox('setValue','');
						$($('#dg1').treegrid('getEditor', {id:editRow.id,field:'warMaterialId'}).target).textbox('setValue','');
  	...
  	function doubleClickRow(row){
    	...
    	if (flag){
    		if (row.isMaterial == 'true'){
				...
				$($('#dg1').treegrid('getEditor', {id:editRow.id,field:'useUnit'}).target).textbox('disable');
				...
				$($('#dg1').treegrid('getEditor', {id:editRow.id,field:'useUnit'}).target).textbox('enable');
				$('#dg1').treegrid('disableDnd');
  		...
  		if(children.length == 0){
			var isMaterialEd = $('#dg1').treegrid('getEditor', {id:id,field:'isMaterial'});
	  		...
	  		if (isMaterial == 'false'){
				...
				$($('#dg1').treegrid('getEditor', {id:id,field:'useUnit'}).target).textbox('disable');
				...
				findWarMaterialByMaterialCode($(ed.target).textbox('textbox').val());
				...
				$($('#dg1').treegrid('getEditor', {id:id,field:'useUnit'}).target).textbox('enable');
