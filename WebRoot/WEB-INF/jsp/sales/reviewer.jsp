  ...
  <script type="text/javascript">
	...
	$(function () {
		$('#dg').treegrid({
		    ...
		    url:'${pageContext.request.contextPath}/sales/reviewer/findAllSalReviewerList.action',    
		    idField:'id',    
		    ...
		    onBeforeExpand:function(row){
		    	//动态设置展开查询的url 
		    	var url = '${pageContext.request.contextPath}/sales/reviewer/findAllSalReviewerSubsidiaryListBySalReviewerId.action'; 
                $(this).treegrid("options").url = url;  
                return true; 
            },onExpand : function(row){
            	//展开后将url设置为原来的url，否则刷新的时候会使用更改后的url刷新
            	var url = '${pageContext.request.contextPath}/sales/reviewer/findAllSalReviewerList.action'; 
                $(this).treegrid("options").url = url;  
	...
	function edit(){
		...
		if (selected != null){
			...
			$.ajax({
				type:'post',
				url:'${pageContext.request.contextPath}/sales/reviewer/findSalReviewerByIdForEdit.action?id='+id,
				async: false,//同步请求
				cache: false,//不缓存此页面
	...
	function view(){
		...
		if (selected != null){
			...
			$.ajax({
				type:'post',
				url:'${pageContext.request.contextPath}/sales/reviewer/findSalReviewerByIdForView.action?id='+id,
				async: false,//同步请求
				cache: false,//不缓存此页面
				success:function(data){
					if (data.success){
	...
	function copyData(){
		...
		if (selected != null){
			...
			$.ajax({
				type:'post',
				url:'${pageContext.request.contextPath}/sales/reviewer/findSalReviewerByIdForCopy.action?id='+id,
				async: false,//同步请求
				cache: false,//不缓存此页面
				success:function(data){
					if (data.success){
	...
	function save(){
		if ($("#form1").form('validate')){//判断 验证是否通过
			var url = "${pageContext.request.contextPath}/sales/reviewer/";
  			...
  			$.messager.confirm('确认对话框', '您确定要删除吗？', function(b){//给用户一个删除几条信息的确认提示框
				if (b){//如何用户点击的是确认
					...
					if(processInstanceId == ""){
			  			$.ajax({
							type:'POST',//post请求
							url: "${pageContext.request.contextPath}/sales/reviewer/deleteSalReviewerById.action",//ajax请求的url
	...
	function exportWord(){
		...
		if (selected==null){
			...
			window.open('${pageContext.request.contextPath}/sales/reviewer/exportWord.action?id=' + selected.id);
		}
	}
	...
	function addFile(){
		...
		if ($("#e6Form").form('validate')){//判断 验证是否通过
			...
			$.ajax({
				type:'POST',//post请求
				url: "${pageContext.request.contextPath}/sales/reviewer/findDocInfo.action",//ajax请求的url
	...
	function downLoadE6File(){
		...
		if (selected==null){
			...
			$("#E6").textbox("disableValidation");
			if ($("#e6Form").form('validate')){//判断 验证是否通过
				var url = '${pageContext.request.contextPath}/sales/reviewer/downLoadE6File.action?docId=' + selected.docId;
				document.getElementById("e6Form").action=url;//设置表单的url，该表单的target设置为_blank,为新窗口打开
				$("#e6Form").submit();//提交表单
				$("#username").textbox("disableValidation");
				$("#password").textbox("disableValidation");
	...
	function queryAuditor(){
  		...
  		if (selected != null){
			...
			if(processInstanceId == "" || processInstanceIdInput.text() == "审核中"){
	  			$.ajax({
					type:'POST',//post请求
					url: "${pageContext.request.contextPath}/sales/reviewer/queryAuditorById.action",//ajax请求的url
					data: {id:id},//传的参数,序列化表单
					async: false,//同步请求
					cache: false,//不缓存此页面
					success: function(data){//请求成功后的回调函数。data：服务器返回数据。
							...
							if (data.rows.length>0){//表示还有下一节点
								...
								$.messager.confirm('确认对话框', '您确定要审核吗？', function(b){
									if (b){//如何用户点击的是确认
										$.ajax({
											type:'POST',//post请求
											url: "${pageContext.request.contextPath}/sales/reviewer/auditReviewer.action",//ajax请求的url
											data: {id:id},//传的参数
											async: false,//同步请求
											cache: false,//不缓存此页面
											success: function(data){//请求成功后的回调函数。data：服务器返回数据。
			  ...
			  if (processInstanceId == ""){
				  ...
				  $.messager.confirm('确认对话框', '您确定要撤销本流程吗？本流程的所有痕迹将会清除', function(b){
					  if (b){//如何用户点击的是确认
							$.ajax({
								type:'POST',//post请求
								url: "${pageContext.request.contextPath}/sales/reviewer/revokeProcess.action",//ajax请求的url
								data: {id:id},//传的参数
								async: false,//同步请求
								cache: false,//不缓存此页面
								success: function(data){//请求成功后的回调函数。data：服务器返回数据。
			...
			if ($("#takeBackForm").form("validate")){
				$.messager.confirm('确认对话框', '确定要收回审核吗？', function(b){
					if (b){//如何用户点击的是确认
						$.ajax({
							type:'POST',//post请求
							url: "${pageContext.request.contextPath}/sales/reviewer/takeBackReviewer.action",//ajax请求的url
		...
		if ($("#rollBackForm").form("validate")){
			$.messager.confirm('确认对话框', '确定要退回审核吗？', function(b){
				if (b){//如何用户点击的是确认
					$.ajax({
						type:'POST',//post请求
						url: "${pageContext.request.contextPath}/sales/reviewer/rollBackReviewer.action",//ajax请求的url
		...
		if ($("#antiAuditForm").form("validate")){
			$.messager.confirm('确认对话框', '确定要反审核吗？', function(b){
				if (b){//如何用户点击的是确认
					$.ajax({
						type:'POST',//post请求
						url: "${pageContext.request.contextPath}/sales/reviewer/antiAuditReviewer.action",//ajax请求的url
  ...
  <body>
  	...
  	<div id="gridToolBar" style="padding:5px;height:auto;">
		<div style="display:inline-block;">
			...
			<p:isPrivilege pid="de" mid="dek">
				<a href="#" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="generateWorkCard();" style="float: left;">生成工咭</a><div class="btn-separator"></div>
			</p:isPrivilege>
		</div>
		<div>
			<form id="queryForm">
	...
	<div id="menu" class="easyui-menu" style="width: 80px; display: none;">
	   ...
	   <p:isPrivilege pid="de" mid="dei">
    	...
    	<p:isPrivilege pid="de" mid="dej">
   		...
   		<p:isPrivilege pid="de" mid="dek">
   			<div data-options="iconCls:'icon-add'" onclick="generateWorkCard()">生成工咭</div>
   		</p:isPrivilege>
	</div>
	...
	<div id="dialog1" class="easyui-dialog" style="width: 1000px;height: 100%;text-align: center;"
  	data-options="closed:true,draggable:true,top:0,modal:true,buttons:'#bb1'">
  		<c:if test="${user.departmentId == 4}"><!-- 销售 -->
	  		<jsp:include page="reviewerFormSales.jsp"></jsp:include>
		</c:if>
		<c:if test="${user.departmentId == 5}"><!-- 工程 -->
			<jsp:include page="reviewerFormEngineering.jsp"></jsp:include>
		</c:if>
		<c:if test="${user.departmentId == 7}"><!-- 采购 -->
			<jsp:include page="reviewerFormPurchase.jsp"></jsp:include>
		</c:if>
		<c:if test="${user.departmentId == 1 || user.departmentId == 3}"><!-- 生产/pmc -->
			<jsp:include page="reviewerFormProductionOrPMC.jsp"></jsp:include>
		</c:if>
		<c:if test="${user.username == '系统管理员'}"><!-- 内置管理员 -->
			<jsp:include page="reviewerFormSystemManager.jsp"></jsp:include>
		</c:if>
	</div>
	<!-- dialog上面dialog的按钮 -->
