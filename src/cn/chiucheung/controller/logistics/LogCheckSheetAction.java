...
@Controller
@RequestMapping("/logistics/checkSheet")
public class LogCheckSheetAction {
	
	@Autowired
	private CheckSheetService checkSheetService;
	
	@RequestMapping("index")
	@AnnotationLimit(mid="hk",pid="hh")
	public String index(){
		return "logistics/checkSheet";
	}
	...
	public Map<String, Object> findAllCheckSheetList(FinCheckSheetQueryVo checkSheetQueryVo){
		...
		checkSheetQueryVo.setIsLock(false);
	 ...
	 * @return
	 */
	@RequestMapping("deleteCheckSheetById")
	@ResponseBody
	@AnnotationLimit(mid="hka",pid="hk")
	public JSONObject deleteCheckSheetById(String id){
		JSONObject jsonObject = new JSONObject();
		try{
			checkSheetService.deleteCheckSheetById(id);
	...
	@ResponseBody
	@AnnotationLimit(mid="hkb",pid="hk")
	 ...
	 * @return
	 */
	@RequestMapping("exportExcelForLiveById")
	@ResponseBody
	@AnnotationLimit(mid="hkb",pid="hk")
	public String exportExcelForLiveById(String id, HttpServletRequest request, HttpServletResponse response){
		try {
			List<ExportExcelForLiveCustom> list = checkSheetService.exportExcelForLiveById(id);
			List<Map<String, List<Object>>> objData = new ArrayList<Map<String,List<Object>>>();
			int i=1;
			for (ExportExcelForLiveCustom data : list) {
				List<Object> list2 = new ArrayList<Object>();
				SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
				list2.add(dateFormat.format(data.getTravelDate()));
				list2.add(data.getHotel());
				list2.add(data.getDormitory());
				list2.add(data.getOther());
			...
			//提供下载
			String filename = "住宿表_" + new Date().toLocaleString()+".xls";
			//response.setHeader("Content-Disposition", "attachment;filename="+URLEncoder.encode(filename, "utf-8"));
	...
	@ResponseBody
	@AnnotationLimit(mid="hkc",pid="hk")
	...
	@RequestMapping("updateCheckSheetByIdForSubmitToFinance")
	@ResponseBody
	@AnnotationLimit(mid="hkd",pid="hk")
	public JSONObject updateCheckSheetByIdForSubmitToFinance(String id){
		JSONObject jsonObject = new JSONObject();
		try{
			checkSheetService.updateCheckSheetByIdForSubmitToFinance(id);
			jsonObject.accumulate("success", true);
		}catch (Exception e){
			e.printStackTrace();
			jsonObject.accumulate("success", false);
