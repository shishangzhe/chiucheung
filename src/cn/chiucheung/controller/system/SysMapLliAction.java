...
import cn.chiucheung.utils.ChineseToEnglish;
import cn.chiucheung.utils.UUIDBuild;

@Controller
@RequestMapping("/system/mapLli")
public class SysMapLliAction implements Serializable{
	
	@Autowired
	MapLliService mapLliService;
	...
	@AnnotationLimit(mid="bg",pid="bb")
	public String index(){
		return "system/mapLli";
	 ...
	 * @return
	 */
	@RequestMapping(value="findAllMapLliList")
	@ResponseBody
	@AnnotationLimit(mid="bg",pid="bb")
	public Map<String, Object> findAllMapLliList(SysMapLliQueryVo mapLliQueryVo){
		Map<String, Object> map = new HashMap<String, Object>();
		try {
			List<SysMapLli> list =  mapLliService.findAllMapLliList(mapLliQueryVo);
			String total = mapLliService.findAllMapLliTotal(mapLliQueryVo);
	 ...
	 * @return
	 */
	@RequestMapping(value="checkMapNameIsRepeat")
	@ResponseBody
	@AnnotationLimit(mid="bga",pid="bg")
	public String checkMapNameIsRepeat(SysMapLli mapLli){
		List<SysMapLli> list = mapLliService.checkMapNameIsRepeat(mapLli);
		String message = "";
	 ...
	 * @return
	 */
	@RequestMapping(value="saveSysMapLli")
	@ResponseBody
	@AnnotationLimit(mid="bga",pid="bg")
	public JSONObject saveSysMapLli(SysMapLli mapLli){
		JSONObject jsonObject = new JSONObject();
		try{
			mapLliService.saveSysMapLli(mapLli);
	 ...
	 * @return
	 */
	@RequestMapping(value="findSysMapLliById")
	@ResponseBody
	@AnnotationLimit(mid="bgb",pid="bg")
	public Map<String, Object> findSysMapLliById(String id){
		Map<String, Object> map = new HashMap<String, Object>();
		try{
			SysMapLli mapLli = mapLliService.findSysMapLliById(id);
			map.put("success", true);
			map.put("mapLli", mapLli);
		}catch(Exception e){
	 ...
	 * @return
	 */
	@RequestMapping(value="updateSysMapLli")
	@ResponseBody
	@AnnotationLimit(mid="bgb",pid="bg")
	public JSONObject updateSysMapLli(SysMapLli mapLli){
		JSONObject jsonObject = new JSONObject();
		try{
			mapLliService.updateSysMapLli(mapLli);
			jsonObject.accumulate("success", true);
		}catch(Exception e){
			jsonObject.accumulate("success", false);
			jsonObject.accumulate("message", e.getMessage());
	 ...
	 * @return
	 */
	@RequestMapping(value="deleteSysMapLliById")
	@ResponseBody
	@AnnotationLimit(mid="bgc",pid="bg")
	public JSONObject deleteSysMapLliById(String id){
		JSONObject jsonObject = new JSONObject();
		try{
			mapLliService.deleteSysMapLliById(id);
			jsonObject.accumulate("success", true);
		}catch(Exception e){
			jsonObject.accumulate("success", false);
			jsonObject.accumulate("message", e.getMessage());
	...
	@ResponseBody
	public String insertList2(){
		try {
			ObjectInputStream ois = new ObjectInputStream(new FileInputStream("d://1.out"));
			List<SysMapLli> list = (List<SysMapLli>) ois.readObject();
			for (SysMapLli mapLli : list) {
				mapLliMapper.insert(mapLli);
			}
		}  catch (Exception e) {
			e.printStackTrace();
	...
	/*public List<SysMapLli> getMapLlisByJson(String fileName,String parent){
		List<SysMapLli> list = new ArrayList<SysMapLli>();
		String path = "C:\\Users\\adm-03\\Desktop\\酒店发票\\incubator-echarts-master\\incubator-echarts-master\\map\\json\\";
		try {
	        ...
	        for (int i = 0; i < jsonArray.size(); i++) {
	        	...
	        	mapLli.setName(name);
	        	if (name.equals("西藏")){
	        		mapLli.setEngName("xizang");
	        	}else if (name.equals("陕西")){
	        		mapLli.setEngName("shanxi1");
	        	}else if (name.equals("重庆")){
	        		mapLli.setEngName("chongqing");
	        	}else{
	        		mapLli.setEngName(ChineseToEnglish.getFullSpell(name));
	        	}
	        	mapLli.setParent(parent);
	        	if (jsonObject2.containsKey("cp")){
		        	String lli = jsonObject2.getString("cp");
		        	lli = lli.substring(1, lli.length()-1);
		        	mapLli.setLongitude(new BigDecimal(lli.split(",")[0]));
		        	mapLli.setLatitude(new BigDecimal(lli.split(",")[1]));
	        	}
	...
	/*public static void main(String[] args) throws Exception {
        ...
        SysMapLliAction sysMapLliAction = (SysMapLliAction) ois.readObject();
        System.out.println(sysMapLliAction);

	    
		SysMapLliAction sysMapLliAction = new SysMapLliAction();
		List<SysMapLli> list = new ArrayList<SysMapLli>();
		List<SysMapLli> chinaList = sysMapLliAction.getMapLlisByJson("china.json", "china");
		
		list.addAll(chinaList);
		for (SysMapLli mapLli : chinaList) {
			List<SysMapLli> provinceList = sysMapLliAction.getMapLlisByJson("province\\" + mapLli.getEngName() + ".json", mapLli.getName());
			list.addAll(provinceList);
		}
		
		ObjectOutputStream oos=new ObjectOutputStream(new FileOutputStream("d://1.out"));
